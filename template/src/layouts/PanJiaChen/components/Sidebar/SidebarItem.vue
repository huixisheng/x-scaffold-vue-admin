    ...
    <template v-for="item in routes" v-if="!item.hidden && item.children">
      ...
      <router-link v-if="hasOneShowingChildren(item.children) && !item.children[0].children&&!item.alwaysShow" :to="item.path + '/' + item.children[0].path"
        ...
        <el-menu-item :index="item.path + '/' + item.children[0].path" :class="{'submenu-title-noDropdown':!isNest}">
          ...
          <span v-if="item.children[0].meta && item.children[0].meta.title" slot="title" v-text="generateTitle(item.children[0].meta.title)"></span>
        </el-menu-item>
      </router-link>
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          ...
          <router-link v-else :to="item.path + '/' + child.path" :key="child.name">
            <el-menu-item :index="item.path + '/' + child.path">
              ...
              <span v-if="child.meta && child.meta.title" slot="title" v-text="generateTitle(child.meta.title)"></span>
            </el-menu-item>
          </router-link>
        </template>
      </el-submenu>
